...
struct My_Discussion: View {
    @State var DiscussModal = false
                ...
                VStack{
                    ZStack{
                        Button{
                            ...
                            }.sheet(isPresented: $DiscussModal, content: {
                                Discussion_Detail()               })
                        }
                        HStack{
                            Text("2,5K").font(.system(size: 13)).foregroundColor(.black)
                            Image(systemName: "hand.thumbsup.circle").resizable().scaledToFit().foregroundColor(.black).frame(width: 20)
                        }.position(x:307, y:110)
...
struct My_Discussion_Previews: PreviewProvider {
